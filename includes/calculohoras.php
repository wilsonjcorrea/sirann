<?php
// informacion de las consultas citadas en el while viene de pagina
$totalhed=0;
$totalhen=0;
$totalrn=0;
$totalrd=0;
...
while($fila = mysqli_fetch_object($result1) and $fila2 = mysqli_fetch_object($result2)){
  ?>
    ...
    <!-- Linea oculta formulario que envia el id de linea consulta principal-->
    <tr>
    <?php $array_id= array('id');?>
    <input type="hidden" value="<?php echo $fila->id;?>" name="id[]">
    <!-- Columna muestra docuemnto de identidad del consultado-->                
    <td><?php echo $fila->id_doc;?></td>
    <!-- Columna muestra la fecha de evaluacion de las condiciones-->
    <td><?php 
      $fecha = $fila->FechaHora;
      $Fecha = substr($fecha, 0, 10);
      echo $Fecha;
    ?></td>
    <!-- Columna muestra el dia de la semana correspondiente a la fecha, resalta los domingos y festivos en verde-->
    <?php
    ...
    <!-- Columna muestra la hora de ingreso de la cedula y fecha-->
    <td><?php 
    ...
    <!-- Columna muestra la hora de salida de la cedula y fecha-->
    <td><?php
    ...
    <!-- Columna muestra el calculo de la cantidad de horas laboradas
        teniendo en cuenta el turno, la hora de almuerzo, o si pasa de dia-->
    <?php
      ...
      if (floatval($thini)>floatval($hprog)){
        ...
        ?><td style="background-color:yellow" ><?php
        $Horas= floatval($horas)- floatval($Alm);
        echo floatval(round($Horas,2));
      }
      else{
        //se valida las horas totales son menores a las horas programadas y se cambia el fondo a naranja
        $Horas= floatval($horas)- floatval($Alm);
        if (floatval($hxt)>floatval($Horas)){
          ?><td style="background-color:orange" ><?php
          echo floatval(round($Horas,2));
        }
        else{
          //se muestra la cantidad neta de horas descontando el tiempo respectivo de almuerzo
    ...
    <!-- Columna muestra el calculo de horas extra diurnas
         realizadas en el dia y usuario evaluado-->
    <td><?php
        ...
        if(floatval($thfin)>floatval($rnini)){//aplica hasta las 23:59:59
          ...
          if(floatval($thini)>floatval($rnfin)){//aplica desde las 6 a.m
            ...
            if (floatval($hed)>0){
              ...
              $totalhed= $totalhed + floatval(round($hed,2));           
            }
            else{
              echo 0;
            }
    ...
    <!-- Columna muestra el calculo de horas extra nocturnas
         realizadas en el dia y usuario evaluado-->
    <td><?php 
      //se llaman variables de total de horas netas generadas y se compara con las horas programadas por turno
      if(floatval($Horas)>floatval($hxt)){
          ...
          if(floatval($thini)>floatval($rnfin)){//aplica desde las 6 a.m
            ...
            if(floatval($hed1)>0){
              ...
              if (floatval($hen)>0){
                ...
                $totalhen= $totalhen + floatval(round($hen,2));
              }
              else{
                echo 0;
              }
    ...
    <!-- Columna muestra el calculo de recargos nocturnos
     realizadas en el dia y usuario evaluado-->
<td><?php 
  //se llaman variables de total de horas netas generadas y se compara con las horas programadas por turno
  if(floatval($Horas)>floatval($hxt)){
    ...
    if(floatval($thfin)>floatval($rnini)){//aplica hasta las 23:59:59
      ...
      if(floatval($thini)>floatval($rnfin)){//aplica desde las 6 a.m
        ...
        //Siempre que la hora de salida fue despues de la hora inicio de recargos noct,
        if(floatval($hed1)>0){
          //si existen horas extra diurnas, el tiempo despues del RECN son extras nocturnas
          echo 0;
          }
        //No hay horas extra diurnas
        //Siempre que la hora de entrada es mayor a la finalizacion de recago nocturno
        //Siempre que la hora de salida fue despues de la hora inicio de recargos noct,
          ...
          //se calcula el tiempo realizado despues del inicio del recargo nocturno
          $trn=floatval($thfin)-floatval($rnini);
          //se calcula si hay horas extra Nocturnas realizadas y se descuentan
          $hen=(floatval($Horas)-floatval($hxt));
          if(floatval($hen)<floatval($trn)){
            $tdrn=floatval($trn)-floatval($hen);
            if($tdrn>0){
              echo floatval(round($tdrn,2));
              $totalrn= $totalrn + floatval(round($tdrn,2));
            }
            else{
              echo 0;
            }
            ...
            //las horas extra nocturnas son iguales a los recargos nocturnos
            echo 0;
          }
        }
      }
      
      //Siempre que la hora de entrada es menor a la finalizacion de recago nocturno/de 0 a 6 am
      //Siempre que la hora de salida fue despues de la hora inicio de recargos noct/aplica hasta las 23:59:59
      //Siempre que las horas generadas son mayores a las horas programadas por turno.
      else{
        //se calcula recargos Nocturnos de la mañana
        $rnam=(floatval($rnfin)-floatval($thini));//aplica despues de las 12 a.m.
        if($rnam>0){
          echo floatval(round($rnam,2));
          $totalrn= $totalrn + floatval(round($rnam,2));
        }
        else{
          echo 0;
        }
    ...
    //Siempre que la hora de salida fue antes de la hora inicio de recargos noct
    //Siempre que las horas generadas son mayores a las horas programadas por turno.
    else{
      //se valida si hora de salida es mayor a hora de entrada.
      //Siempre que la hora de salida fue antes de la hora inicio de recargos noct
      //Siempre que las horas generadas son mayores a las horas programadas por turno.
      if(floatval($thfin)>floatval($thini)){
        //se valida si el inicio del recarho nocturno es mayor a las horas por turno
        //siempre que la hora de salida es mayor a hora de entrada.
        //Siempre que la hora de salida fue antes de la hora inicio de recargos noct
        //Siempre que las horas generadas son mayores a las horas programadas por turno.
        if(floatval($rnini)>floatval($hxt)){
          $rnam=(floatval($rnfin)-floatval($thini));
          if($rnam>0){
            echo floatval(round($rnam,2));
            $totalrn= $totalrn + floatval(round($rnam,2));
          }
  ...
  //siempre que la horas totales son igual o menores a las horas programadas por turno
  else{
    //se valida si la hora de entrada es mayor al inicio del recargo nocturno
    //siempre que la horas totales son igual o menores a las horas programadas por turno  
    if(floatval($thini) > floatval($rnini)){
      //Se valida que la hora des salida es menor a finalizacion del recargo nocturno
      //siempre que la hora de entrada es mayor al inicio del recargo nocturno
      //siempre que la horas totales son igual o menores a las horas programadas por turno
      if(floatval($thfin) <= floatval($rnfin)){
        $tdrn=floatval($Horas);
      ...
      //siempre que la horas totales son igual o menores a las horas programadas por turno
        ...
        //Se calcula la diferencia entre la hora de finaclizacion del recargo Noc y la hora de salida.
        //se descuenta del total de horas.
        $tdrn=(floatval($Horas)-((floatval($thfin))-(floatval($rnfin))));
        if(floatval($tdrn)>0){
          echo floatval(round($tdrn,2));
          $totalrn= $totalrn + floatval(round($tdrn,2));
        }
    ...
    //Siempre que la hora de entrada es menor al inicio del recargo nocturno
    //siempre que la horas totales son igual o menores a las horas programadas por turno
    else{
      //Se valida que la hora de salida sea mayor a la hora de entrada.
      //Siempre que la hora de entrada es menor al inicio del recargo nocturno.
      //siempre que la horas totales son igual o menores a las horas programadas por turno.
      if(floatval($thini)<(floatval($thfin))){
        $tdrn=((floatval($thfin))-(floatval($rnini)));
        if(floatval($tdrn)>0){
          echo floatval(round($tdrn,2));
          $totalrn= $totalrn + floatval(round($tdrn,2));
      ...
      //Siempre que la hora de salida sea menor a la hora de entrada.
      //Siempre que la hora de entrada es menor al inicio del recargo nocturno.
      //siempre que la horas totales son igual o menores a las horas programadas por turno.
      else {
        //Se valida si la hora de salida es menor a la hora de finalizacion del recargo nocturno.
        //Siempre que la hora de salida sea menor a la hora de entrada.
        //Siempre que la hora de entrada es menor al inicio del recargo nocturno.
        //siempre que la horas totales son igual o menores a las horas programadas por turno.
        if(floatval($thfin)<floatval($rnfin)){
          $tdrn=((floatval($thfin)+24)-(floatval($rnini)));
          if(floatval($tdrn)>0){
            echo floatval(round($tdrn,2));
            $totalrn= $totalrn + floatval(round($tdrn,2));
          }
        ...
        //Siempre que la hora de salida es mayor a la hora de finalizacion del recargo nocturno.
        //Siempre que la hora de salida sea menor a la hora de entrada.
        //Siempre que la hora de entrada es menor al inicio del recargo nocturno.
        //siempre que la horas totales son igual o menores a las horas programadas por turno. 
        else{
          $tdrn=((floatval($rnfin)+24)-(floatval($rnini)));
          if(floatval($tdrn)>0){
            echo floatval(round($tdrn,2));
            $totalrn= $totalrn + floatval(round($tdrn,2));
          }
  ...
  ?></td>  
    ...
    <td style="background-color:gray"><?php 
      echo floatval(round($thini,2));
    ?></td>
    <td  style="background-color:blue"><?php
